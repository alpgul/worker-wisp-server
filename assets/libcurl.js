...
if (!url.endsWith("/")) url += "/";
url += parts[0] + ":" + port;if(SOCKFS.websocketArgs["url"]){var parts = addr.split("/");
url = Module.websocket.url;
if (!url.endsWith("/")) url += "/";
url += parts[0] + ":" + port;}if(SOCKFS.websocketArgs["subprotocol"]){subProtocols=SOCKFS.websocketArgs["subprotocol"]}else if(SOCKFS.websocketArgs["subprotocol"]===null){subProtocols="null"}if(url==="ws://"||url==="wss://"){var parts=addr.split("/");url=url+parts[0]+":"+port+"/"+parts.slice(1).join("/")}if(subProtocols!=="null"){subProtocols=subProtocols.replace(/^ +| +$/g,"").split(/ *, */);opts=subProtocols}var WebSocketConstructor;{WebSocketConstructor=WebSocket}try {
...
class WispWebSocket extends EventTarget {
  constructor(url, protocols) {
    ...
    if (wsproxy_path[0] === "" && url_split.length > 1) wsproxy_path = url_split.pop().split(":");
    this.host = wsproxy_path[0];
    this.port = parseInt(wsproxy_path[1]);
    if (this.host && typeof DNS !== "undefined" && DNS.address_map.names[this.host]) {
      this.host = DNS.address_map.names[this.host];
    }
    this.real_url = websocket_url;
