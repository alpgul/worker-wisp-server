...
| `WISP_COALESCE_MAX`     | `65536` | tcp→ws wire-coalescing batch cap. The tcp reader accumulates small chunks into one DATA packet (amortizing ws per-message framing overhead for bulk transfers); flush also happens when the downstream queue is already full or after `WISP_COALESCE_TIMEOUT` ms of quiet. Interactive latency is only ever a single coalesce window per burst, never one per chunk. |
| `WISP_COALESCE_TIMEOUT` | `10` | How long (ms) the coalescer holds a below-cap batch before flushing it. `0` disables coalescing entirely (every tcp chunk is relayed as its own DATA packet, the historical behavior). |
...
Bulk transfers arrive at the tcp reader as many small chunks; each one used to become its own WebSocket message (7+ bytes of ws framing + a 5-byte wisp header per message). `WISP_COALESCE_MAX` / `WISP_COALESCE_TIMEOUT` batch these chunks on the server side into single DATA packets: the reader parks chunks and flushes them as one packet when the batch fills, when the downstream queue is already full (backpressure takes over and the stall bound keeps counting queued packets, so memory stays bounded by `downstream_buffer` packets plus one batch per stream), or after a short quiet window. The flush window is the only added latency and it applies per burst — a single chunk still goes out immediately when nothing else is in flight — so interactive traffic is not inflated per packet. `WISP_COALESCE_TIMEOUT=0` restores the historical one-packet-per-chunk behavior. The wire format is unchanged (each WS message still carries exactly one packet, so this interoperates with any wisp client).
